    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            categories.forEach(category => {
                let categoryCard = `
                <div onclick="categoryDetail('${category.strCategory}')" id="${category.idCategory}" class="relative hover:cursor-pointer overflow-hidden md:w-60 w-44 group rounded-xl shadow-md duration-300 text-slate-500 shadow-slate-200">
                    <figure class="relative border-2 border-slate-200">
                        ...
                        <figcaption class="absolute bottom-0 left-0 w-full p-4 text-bottom text-white">
                            <h3 class="text-2xl font-semibold transition-colors line-clamp-2 overflow-hidden group-hover:text-white">${category.strCategory}</h3>
                        </figcaption>
                    </figure>
                </div>
            `;
